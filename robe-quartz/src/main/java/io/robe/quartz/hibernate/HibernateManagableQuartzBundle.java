...
//            List<TriggerEntity> triggerEntries = quartzJob.getTriggers();
//            Set<Trigger> triggers = new LinkedHashSet<Trigger>();
//            for (TriggerEntity triggerEntity : triggerEntries) {
//                TriggerBuilder<Trigger> trigger = newTrigger();
//                String cron = triggerEntity.getCronExpression();
//                if (cron != null && cron.trim().length() > 0) {
//                    trigger.startAt(new Date(triggerEntity.getFireTime())).withSchedule(CronScheduleBuilder.cronSchedule(cron));
//                    LOGGER.info("Trigger set to start at" + new Date(triggerEntity.getFireTime()) + " with this cron definition : " + cron);
//                } else
...
//                for (TriggerEntity triggerEntity : triggerEntries) {
//                    LOGGER.info("Scheduled job : " + job.toString() + " with trigger : " + triggerEntity.getCronExpression() + " started at : " + new Date());
//                }
//            }
//        } else {
...
//        List<TriggerEntity> triggerEntityList = session.createCriteria(TriggerEntity.class)
//                .add(Restrictions.eq("jobId", quartzJob.getOid()))
//                .add(Restrictions.eq("active", true)).list();
//        if (!triggerEntityList.isEmpty()) {
//            for (TriggerEntity quartTrigger : triggerEntityList) {
//                quartTrigger.setActive(false);
//                session.persist(quartTrigger);
//            }
//        }
//        LOGGER.info("All triggers belongs to : " + quartzJob.getClazz() + " deactivated..!");
//    }
//
//    /**
...
//
//    /**
//     * If that job parameter exist in database gets it
//     *
